		...
		void DoSetVelocity()
		{
			...
			if (vector.IsNone)
			{
				velocity = space == Space.World ?
					...
					go.transform.InverseTransformDirection(rigidbody.linearVelocity);
			}
			else
			{
			...
			rigidbody.linearVelocity = space == Space.World ? velocity : go.transform.TransformDirection(velocity);
		}
	}
}